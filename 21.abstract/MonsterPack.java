...
public class MonsterPack
{
    ...
    public void loadMonsters( )
    {
        ...
        for(int j=0; j<monsters.length; j++)//polymorphism
        {
            out.print("Enter Monster " + j + " Name :: ");
            int r = (int)(Math.random()*3);
            if(r==0)
                monsters[j] = new Vampire(keyboard.nextLine()); //each one takes in a String Name
            else if( r==1 )
    ...
    public String toString( )
    {
        String output="";
        for(int j=0; j < monsters.length; j++)
            output+=monsters[j].toString(); //return name + "  says  " + talk();
        return output;
    }
}